...
SNR_dB = 0:4:20; 
Sig_Power = 1e0;
Pilot_Power = Sig_Power.*Rho;
Data_Power = Sig_Power.*(1-Rho);

% Sig_Power = 1e0;
% Pilot_Power1 = Rho./ P;
% Pilot_Power = Rho./ P * N;
% Data_Power = (Sig_Power - P.*Pilot_Power1)./(N - P + P.*(1 - Rho)*(1 - Rho) ) * N;
...
%---读取COST2100产生的信道状态信息------------        
        load('H_Fre256MPCs8_0.6Mhz.mat');
        Num_temp = size(H_Fre); 
%         Num_ = round(Num_temp(1)./length(SNR_dB));
Num_ = 100;
global H_source;  
    ...
    tic
            ...
            %--- 生成 发射信号X -----
                    [X,s] = Gen_Txdata(N,tao,K,I,J,W,Data_Power,Pilot_Power,c_train);
                    s_save(nnnn,:,:) = s;
            %--- 信号X过信道(无线+AWGN) ---     
                    [H_source,Y] = through_channel(N,Noise_Power(cccc),X,L,F,H_Fre(nnnn,:,:));
...
%                         H_SourceNorm = H_source(:,iiii)/norm(H_source(:,iiii));
                        temp_MSE_LS(iiii) =(norm(H_ls(:,iiii)-H_source(:,iiii)))^2/(norm(H_source(:,iiii))^2);
                        temp_MSE_MMSE(iiii) =(norm(H_mmse(:,iiii)-H_source(:,iiii)))^2/(norm(H_source(:,iiii))^2);   
                    end
            ...
            for iiii = 1:test_time
                  ...
                  NN_LS_Equali_MMSE_save(nnnn,:,:) = After_iter_data111;
            ...
            for iiii = 1:test_time
                  ...
                  NN_MMSE_Equali_MMSE_save(nnnn,:,:) = After_iter_data222;
    ...
    toc
            ...
            %---保存用于网络训练数据---
%             save('teData/L_88/Rho3/teNN_H_LS'+string(SNR_dB(cccc))+'.mat','H_ls_save');                        %——经LS估计后得到的CSI
%             save('teData/L_88/Rho3/teNN_H_MMSE'+string(SNR_dB(cccc))+'.mat','H_mmse_save'); 
%             save('teData/L_88/Rho3/teNN_TabH'+string(SNR_dB(cccc))+'.mat','H_source_save');                    %——源CSI
%             save('teData/L_88/Rho3/teNN_Rxdata'+string(SNR_dB(cccc))+'.mat','Y_save');                         %——接收数据
%             save('teData/L_88/Rho3/teNN_LS_Equali_ZF'+string(SNR_dB(cccc))+'.mat','NN_LS_Equali_ZF_save');  
%             save('teData/L_88/Rho3/teNN_LS_Equali_MMSE'+string(SNR_dB(cccc))+'.mat','NN_LS_Equali_MMSE_save'); %——LS估计后经ZF均衡
%             save('teData/L_88/Rho3/teNN_MMSE_Equali_ZF'+string(SNR_dB(cccc))+'.mat','NN_MMSE_Equali_ZF_save'); %——MMSE估计后经ZF均衡
%             save('teData/L_88/Rho3/teNN_MMSE_Equali_MMSE'+string(SNR_dB(cccc))+'.mat','NN_MMSE_Equali_MMSE_save'); %——MMSE估计后经ZF均衡
%             save('teData/L_88/Rho3/teNN_H_Equali_ZF'+string(SNR_dB(cccc))+'.mat','NN_H_Equali_ZF_save');       %——源信道估计后经ZF均衡
%             save('teData/L_88/Rho3/teNN_Tabdata'+string(SNR_dB(cccc))+'.mat','s_save');                        %——发送的经QPSK调制数据
